...
// Nuevo carrusel "Sorprende a los que más quieres" que carga desde Firebase
class SorprendeCarousel {
    constructor() {
        this.currentIndex = 0;
        this.images = [];
        this.itemsVisible = this.getVisibleItems();
        ...
        // Elementos del DOM
        this.container = document.querySelector('.sorprende-carousel-container');
        this.track = document.querySelector('.sorprende-carousel-track');
        this.dotsContainer = document.querySelector('.sorprende-dots');
        this.prevBtn = document.querySelector('.sorprende-prev');
        this.nextBtn = document.querySelector('.sorprende-next');
    ...
    async loadImagesFromFirebase() {
        try {
            const snapshot = await firebase.database().ref('sorprende-imgs').once('value');
            const data = snapshot.val();
            
            if (!data) {
                console.warn('No se encontraron imágenes en sorprende-imgs');
                return;
            }

            // Convertir el objeto a array y ordenar si tiene un campo orden
            this.images = Object.keys(data).map(key => ({
                id: key,
                ...data[key]
            })).sort((a, b) => (a.orden || 0) - (b.orden || 0));

            console.log(`Cargadas ${this.images.length} imágenes del carrusel Sorprende`);
        } catch (error) {
            console.error('Error cargando imágenes desde Firebase:', error);
            // Fallback con algunas imágenes por defecto
            this.images = [
                { id: '1', url: 'https://via.placeholder.com/400x300?text=Error+Cargando', alt: 'Error' }
            ];
        }
    }
    ...
    createCarouselItems() {
        ...
        this.images.forEach((image, index) => {
            ...
            item.className = 'sorprende-carousel-item';
            
            const img = document.createElement('img');
            img.src = image.url || image.imagen;
            img.alt = image.alt || image.descripcion || `Cliente satisfecho ${index + 1}`;
            img.loading = 'lazy';
            img.onerror = () => {
                img.src = 'https://via.placeholder.com/400x300?text=Error+Imagen';
            };
    ...
    getVisibleItems() {
        ...
        if (width < 480) return 1;      // iPhone y móviles pequeños
        if (width < 768) return 2;      // Tablets pequeñas
        if (width < 1024) return 3;     // Tablets grandes
        return 4;                       // Desktop
    }

    navigateTo(index) {
        const maxIndex = Math.max(0, this.images.length - this.itemsVisible);
    ...
    updateCarouselPosition(animate = true) {
        ...
        const translateX = -(this.currentIndex * (100 / this.itemsVisible));
    ...
    updateDots() {
        const dots = this.dotsContainer.querySelectorAll('.sorprende-dot');
    ...
    handleTouchMove(e) {
        ...
        const translateX = -(this.currentIndex * (100 / this.itemsVisible)) - (diff / this.track.offsetWidth * 100);
    ...
    handleTouchEnd(e) {
        ...
        // Umbral mínimo para considerar swipe (20% del ancho visible)
    ...
    handleMouseMove(e) {
        ...
        const diff = this.startPos - e.clientX;
        const translateX = -(this.currentIndex * (100 / this.itemsVisible)) - (diff / this.track.offsetWidth * 100);
    ...
    handleMouseUp(e) {
        ...
        this.isDragging = false;
        const diffX = this.startPos - e.clientX;
    ...
    handleResize() {
        const newItemsVisible = this.getVisibleItems();
    ...
    startAutoScroll() {
        if (!this.isAutoScrolling || this.images.length <= this.itemsVisible) return;
        ...
        }, 3000); // 3 segundos entre transiciones
    }
    ...
    resetAutoScroll() {
        this.stopAutoScroll();
        setTimeout(() => this.startAutoScroll(), 1000); // Esperar 1 segundo antes de reiniciar
    }

    showError() {
        if (this.container) {
            this.container.innerHTML = `
                <div class="sorprende-error">
...
window.SorprendeCarousel = SorprendeCarousel;